...
void VoxelGenerator::Generate() {
    ...
    std::cout << "CPU generated geometry: " << std::endl;
    for (int i = 0; i < 20; i++) {
        glm::vec4 v;
        DoStep(v);
        std::cout << v.x << " " << v.y << " " << v.z << std::endl;
...
void VoxelGenerator::DumpBuffer() {
    ...
    // glm::uvec2 *buffer_data = static_cast<glm::uvec2 *>(m_debug_buffer->GetMappedData());
    std::cout << "Begin dumping buffer" << std::endl;

    // for (uint32_t i = 0; i < 20; i++) {
    //     glm::uvec2 v = buffer_data[i];
    //     glm::uvec3 pos(v.x & 0xfffu, (v.x >> 12u) & 0xfffu, (v.x >> 24u) | ((v.y >> 28u) << 8u));
    //     print_vec(pos);
    // }

    for (uint32_t i = 0; i < 20; i++) {
        glm::vec3 v = glm::vec3(buffer_data[i].x, buffer_data[i].y, buffer_data[i].z);
        print_vec(v);
    }
